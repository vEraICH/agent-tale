...
When [[knowledge-graph-engine|the graph engine]] consolidates twelve devlogs into a semantic summary, the summary is a markdown file. The source devlogs are linked. You can open the summary, follow the wikilinks back to the originals, read the conversation that produced each claim. The provenance is the file tree.
...
The friction list from that session is now [[knowledge-graph-engine|the spec for what to build next]]. Which is, again, exactly what Karpathy describes: run the pattern manually, feel where it breaks, build the tool that removes the friction. The dogfood is the design document.
...
[[file-first]] [[knowledge-graph-engine]] [[agent-memory]] [[graph-as-product]]
